    ...
    if (true == hdl->tx_idle)
    {
...
#endif
        {
            ...
            if (1 < length)
            {
                ...
                if (0x80 == (hdl->rom->regs->SSR.BYTE & 0x80)) /* "SSR.BYTE & 0x80" is "SSR.BIT.TDRE" */
                {
                    SCI_TDR(SCI_CFG_DUMMY_TX_BYTE);    /* start transfer */
                    hdl->tx_cnt--;
                }
...
static void sci_fifo_receive(sci_hdl_t const hdl)
{
    ...
    {
#if (SCI_CFG_ASYNC_INCLUDED)
        fifo_num = hdl->rom->regs->FDR.BIT.R;
                        ...
                        void                *p_args)
{
...
#if SCI_CFG_PARAM_CHECKING_ENABLE
    if ((NULL == hdl)
     || ((NULL == p_args) && (SCI_CMD_CHANGE_BAUD == cmd)))
    {
        return SCI_ERR_NULL_PTR;
    }
    ...
    {
        if ((*(uint8_t *)p_args < 1) || (*(uint8_t *)p_args > 15))
        {
            return SCI_ERR_INVALID_ARG;
        }
    }
