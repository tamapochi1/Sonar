 ...
 ***********************************************************************************************************************/
#pragma inline(R_PDC_GetVersion)
...
static void pdc_callback_frame_end (void * pparam)
{
    ...
    do
    {
        ...
        timeout_count += 1;
    } while (PDC_FIFO_NOT_EMPTY == PDC.PCSR.BIT.FEMPF);
 ...
 * Return Values: none
 ***********************************************************************************************************************/
#pragma interrupt pdc_pcdfi_isr(vect = VECT(PDC, PCDFI))
static void pdc_pcdfi_isr (void)
{
    /* To cast in order to compare the address. There is no problem because the information is not lost even if the
     *  cast. */
